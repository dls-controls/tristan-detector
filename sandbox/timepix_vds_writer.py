...
def create_top_level_VDS_file(filename, src_dir="./", block_size=50000):
    ...
    dcpl.set_virtual(virt_dspace, os.path.join(src_dir, 'test_file_%b.h5'), "/data", src_dspace)

    # Create the virtual dataset
    dset = h5py.h5d.create(f.id, name="data", tid=h5py.h5t.NATIVE_INT32, space=virt_dspace, dcpl=dcpl)
...
class TimepixVdsWriter(object):
    ...
    def execute(self, samples):
        ...
        while not eof:
            ...
            event_qty = self._nx_files[0].event_id_size()[0] + self._nx_files[1].event_id_size()[0]
            ...
            while event_qty > (count + self._block_size):
                ...
                while events_to_read > 0:
                    print("self._nx_files[current_file].event_index_size()[0]", self._nx_files[current_file].event_index_size()[0])
