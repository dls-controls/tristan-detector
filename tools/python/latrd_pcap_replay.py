...
class LATRDPacket(object):
    ...
    TIME_SLICE_MASK  = 0x0003FFFFFFFC0000
...
class LATRDFrameProducer(object):
    ...
    def send_packets(self):
        ...
        for port in self.args.ports:
            send_thread = threading.Thread(target=self._send_packets, args=(int(port),))
            send_threads.append(send_thread)
            send_thread.start()

    def _send_packets(self, port):
        """
        ...
        logging.info("Sending %d data packets in %f seconds", len(self._packets), self.args.duration)
        data_bytes_sent = 0
        data_packets_sent = 0
        delay = float(self.args.duration)/float(len(self._packets))
