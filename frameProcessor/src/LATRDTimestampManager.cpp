...
namespace FrameProcessor {
    ...
    void LATRDTimestampManager::add_timestamp(uint32_t packet_number, uint64_t timestamp) {
        // Protect this method
        boost::lock_guard<boost::recursive_mutex> lock(mutex_);
        LOG4CXX_ERROR(logger_, "add_timestamp called with packet number [" << packet_number << "] timestamp [" << timestamp << "]");
                ...
                if (delta_timestamp_ > 0) {
                    if (delta != delta_timestamp_) {
                        ...
                        LOG4CXX_ERROR(logger_,
                                      "Timestamp [" << timestamp << "] older timestamp [" << timestamps_[packet_number]
                                                    << "]");
                ...
                if (timestamps_.count(index) > 0){
                    ...
                    if (delta > 0) {
                        if (delta_timestamp_ > 0) {
                            if (delta != delta_timestamp_) {
                                // This is potentially a serious error so log it and raise an exception
                                LOG4CXX_ERROR(logger_, "Delta extended timestamp values are not consistent. Old delta [" << delta_timestamp_ << "] new delta [" << delta << "]");
                                LOG4CXX_ERROR(logger_, "Packet number [" << packet_number << "] index [" << index << "]");
                                LOG4CXX_ERROR(logger_, "Timestamp [" << timestamp << "] older timestamp [" << timestamps_[index] << "]");
                                throw LATRDTimestampException("Delta extended timestamp values are not consistent");
                            } else {
                            ...
                            delta_timestamp_ = delta;
                            LOG4CXX_ERROR(logger_, "Packet number [" << packet_number << "] index [" << index << "]");
                            LOG4CXX_ERROR(logger_, "New delta whilst backtracking [" << delta_timestamp_ << "]");
